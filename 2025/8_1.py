...
count0 = len(set_points)

set_points.clear()
for item in sorted_lists[1]:
    set_points.add(item.split(":")[0])
    set_points.add(item.split(":")[1])
count1 = len(set_points)

set_points.clear()
for item in sorted_lists[2]:
    set_points.add(item.split(":")[0])
    set_points.add(item.split(":")[1])
count2 = len(set_points)
print(count0)
print(count1)
print(count2)
# sum = len(sorted_lists[0]) *len(sorted_lists[1])*len(sorted_lists[2])
